...
- [1.前期准备](#前期准备)
  - [启动外部高速时钟](#启动外部高速时钟)
  - [配置时钟树](#配置时钟树)
  - [Debug选择(选择串口)](#debug选择选择串口)
  - [项目管理设置](#项目管理设置)
  - [生成项目所包含的内容](#生成项目所包含的内容)
  - [增加类型别名](#增加类型别名)
  - [模块化编程基本步骤](#模块化编程基本步骤)
  - [.h文件与.c文件标准格式](#h文件与c文件标准格式)
- [2.LED操作(闪烁操作)](#led操作闪烁操作)
- [Key操作(短按键与长按键)](#key操作短按键与长按键)
- [通过按键实现界面的切换](#通过按键实现界面的切换)
- [PWM输出(输出比较)](#pwm输出输出比较)
- [输入捕获(测量频率占空比)](#输入捕获测量频率占空比)
- [模数转换ADC](#模数转换adc)
- [IIC通信(eeprom读写)](#iic通信eeprom读写)
- [UART串口通信](#uart串口通信)
